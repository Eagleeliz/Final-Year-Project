...
const AdminSideNav = ({ onNavItemClick }: NavProps) => {
  ...
  return (
    <nav
      className="flex flex-col h-full w-full px-3 pt-4 text-white overflow-y-auto"
    ...
    >
      ...
      <div className="flex-1 space-y-3 pt-6">
        {navItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center px-4 py-3 rounded-lg transition-all text-white ${
                isActive ? "font-semibold" : ""
              }`
            }
            style={({ isActive }) => ({
              backgroundColor: isActive ? "#004d57" : "transparent",
            })}
          >
            <item.icon
              size={22}
              className="mr-3"
              style={item.name === "Emergency Alerts" ? { color: "#f4b8a0" } : {}}
            />
      ...
      <div className="mt-auto pt-4 border-t border-white/10">
        <button
          ...
          className="flex items-center w-full px-4 py-3 text-white transition hover:bg-white/5 rounded-lg"
        >
