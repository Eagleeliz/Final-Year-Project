  ...
  return (
    <section className="w-full py-20 px-6 overflow-hidden mt-8" style={{ background: bgSection }}>
      ...
      <div className="relative flex flex-col md:flex-row items-start justify-center max-w-5xl mx-auto mb-12 gap-7 md:gap-0">
        <div className="hidden md:block absolute top-16 left-[10%] right-[10%] h-[6px] rounded-full z-0" style={{ background: "linear-gradient(90deg, #00e5f5 0%, #00c6d4 35%, #00a0b0 65%, #005f6b 100%)", boxShadow: "0 2px 12px rgba(0,180,200,0.35)" }} />
        <div className="md:hidden absolute left-[44px] top-0 bottom-0 w-[6px] rounded-full z-0" style={{ background: "linear-gradient(180deg, #00e5f5 0%, #00c6d4 35%, #00a0b0 65%, #005f6b 100%)", boxShadow: "2px 0 12px rgba(0,180,200,0.35)" }} />

        {steps.map((step, i) => (
          <div key={i} className="flex-1 flex md:flex-col flex-row md:items-center items-start relative z-10 px-2 gap-4 md:gap-0">
            <div className={`relative flex items-center justify-center rounded-full flex-shrink-0 w-[88px] h-[88px] md:w-[128px] md:h-[128px] mb-0 md:mb-[18px] transition-transform duration-300 hover:scale-105 cursor-default ${step.bg}`}>
              <div className={`w-[54px] h-[54px] rounded-[14px] flex items-center justify-center ${step.iconBg}`}>{step.icon}</div>
              <span className="absolute bottom-[5px] right-[5px] w-7 h-7 rounded-full text-[12px] font-bold flex items-center justify-center border-2 border-white" style={{ background: midnightTeal, color: "#86d9e1" }}>{step.num}</span>
            </div>
